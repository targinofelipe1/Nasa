// components/ui/MapaApoioHeatmap.tsx
'use client';
...
import { GeoJsonObject, Feature, FeatureCollection, Geometry, GeoJsonProperties } from 'geojson';
...
const coresGradiente = {
    // CORRIGIDO: Tom de vermelho mais claro
    semApoio: '#B71C1C', 
    semDados: '#E8E8E8',
};

const normalizeString = (str: string): string => str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
...
const MapaApoioHeatmap: React.FC<MapaApoioHeatmapProps> = ({ apiData }) => {
  ...
  const onEachFeature = (feature: Feature, layer: L.Layer) => {
    ...
    if (dados) {
        ...
        tooltipContent += `<div>Votos Esperados: ${dados.votos.toLocaleString('pt-BR')}</div>`;
    } else {
