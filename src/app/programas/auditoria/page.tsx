...
import AuditDetailsModal from '@/components/ui/AuditDetailsModal';
...
export default function AuditoriaPage() {
  ...
  const programTitle = programDisplayNames[programName] || 'Log de Auditoria';
  ...
  const matchesMunicipio = String(row['municipio']).toLowerCase().includes(municipioFilter.toLowerCase());
  
  const userName = getUserName(String(row['userId']));
  const matchesUserName = userName.toLowerCase().includes(userIdFilter.toLowerCase());
  ...
  const handleOpenModal = (logEntry: TableData) => {
    console.log('Objeto logEntry que chega no modal:', logEntry);
