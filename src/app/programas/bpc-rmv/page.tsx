// src/app/programas/bpc-rmv/page.tsx
"use client";
...
import BpcRmvUpdateModalContent from "@/components/ui/BpcRmvUpdateModalContent";
...
const programId = "bpc-rmv";
const requiredTab = "BPC/RMV";

export default function BpcRmvPage() {
  ...
  const [activeTab, setActiveTab] = useState<keyof typeof tabGroups>(requiredTab);
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      ...
      try {
        ...
        const localAllowed =
          (user.unsafeMetadata as { allowedTabs?: string[] } | undefined)?.allowedTabs || [];
        const permString = `${programId}_${requiredTab}`;
        const localHas = localAllowed.includes(permString);
        ...
        const serverHas = serverAllowed.includes(permString);

        const finalHas = serverHas || localHas;
