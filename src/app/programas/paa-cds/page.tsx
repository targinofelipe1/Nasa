...
export default function ProgramasPage() {
  ...
  const [beneficiariosData, setBeneficiariosData] = useState<
    { municipio: string; quantidade: number }[]
  >([]);
  const [agricultoresPorMunicipio, setAgricultoresPorMunicipio] = useState<
    { municipio: string; quantidade: number }[]
  >([]);
  const [cooperativasPorMunicipio, setCooperativasPorMunicipio] = useState<
    { municipio: string; quantidade: number }[]
  >([]);
  const [entidadesPorMunicipio, setEntidadesPorMunicipio] = useState<
    { municipio: string; quantidade: number }[]
  >([]);
  const [totalAdquiridoMunicipios, setTotalAdquiridoMunicipios] = useState<
    { municipio: string; totalKG: number }[]
  >([]);
  const [investimentoPorMunicipio, setInvestimentoPorMunicipio] = useState<
    { municipio: string; total: number }[]
  >([]);
  const [agricultoresFiltrados, setAgricultoresFiltrados] = useState<
    Agricultor[]
  >([]);
  ...
  >([]);
  const [ordenacaoEntidade, setOrdenacaoEntidade] = useState<
    'asc' | 'desc'
  >('asc');
  ...
  const [campoPesquisaCompras, setCampoPesquisaCompras] = useState<
    'cooperativa' | 'cnpj'
  >('cooperativa');
  ...
  useEffect(() => {
    fetch('/api/sheets/programas/paa/Agricultores')
      ...
      .then((res) => {
        ...
        const totalMulheres = data.filter(
          (item: LinhaCSV) => item[3]?.toLowerCase() === 'feminino'
        ).length;
        ...
        const compras = data
          ...
          .map((item) => ({
            ...
            valorUnidade: parseFloat(
              item[6].replace(/\./g, '').replace(',', '.')
            ).toLocaleString('pt-BR', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            }),
            total: parseFloat(
              item[7].replace(/\./g, '').replace(',', '.')
            ).toLocaleString('pt-BR', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            }),
    ...
    fetch('/api/sheets/programas/paa/Municipio')
      ...
      .then((res) => {
        ...
        data.forEach((item) => {
          const municipio = item[0]?.trim().toUpperCase();
          const totalKG =
            parseFloat(item[1]?.trim().replace(/\./g, '').replace(',', '.')) || 0;
          const total =
            parseFloat(item[2]?.trim().replace(/\./g, '').replace(',', '.')) || 0;
    ...
    fetch('/api/sheets/programas/paa/Entidades')
      ...
      .then((res) => {
        ...
        const entidadesProcessadas = data
          ...
          .map((item) => {
            const doacaoKg =
              parseFloat(item[2].replace(/\./g, '').replace(',', '.')) || 0;
            const valor =
              parseFloat(item[3].replace('R$', '').replace(/\./g, '').replace(',', '.')) ||
              0;
        ...
        <div className="flex-1 overflow-auto" style={{ zoom: '80%' }}>
            <div className="w-full pt-6 pb-2 bg-white shadow-sm border-b border-gray-200 px-10">
              ...
              <div className="flex space-x-10 mt-5 border-b border-gray-300">
                {['Visão Geral', 'Agricultores', 'Entidades', 'Cooperativas'].map(
                  (tab) => (
                    <button
                      key={tab}
                      onClick={() => {
            ...
            {abaAtiva === 'Visão Geral' && (
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 bg-white rounded-xl p-10">
                  ...
                  <CardPrograma
                    value={mulheres}
                    label="Total de Agricultoras"
                    icon={<FaFemale />}
                    bgColor="bg-purple-100"
                    iconBg="bg-purple-500"
                  />
                  <CardPrograma
                    value={municipiosFornecedor}
                    label="Municípios Fornecedores"
                    icon={<FaMapMarkedAlt />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-10 p-10">
                    ...
                    <div className="flex-grow">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          />
                          <Tooltip
                            formatter={(value: number, name: string, props: any) => [`${value} Beneficiários`, props.payload.municipio]}
                          />
                          <Bar dataKey="quantidade" fill="#3498db" barSize={20} radius={[0, 10, 10, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                  ...
                  <div className="h-[600px] bg-white rounded-xl shadow p-4 flex flex-col">
                    <h2 className="text-lg font-bold text-center mb-2">
                      Total Adquirido (KG) por Município
                    </h2>
                    <div className="flex-grow">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          data={totalAdquiridoMunicipios}
                          layout="vertical"
                          margin={{ top: 5, right: 30, left: 100, bottom: 5 }}
                        >
                          <XAxis type="number" hide />
                          ...
                          />
                          <Tooltip
                            formatter={(value: number, name: string, props: any) => [`${value.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} KG`, props.payload.municipio]}
                          />
                          <Bar dataKey="totalKG" fill="#f39c12" barSize={20} radius={[0, 10, 10, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 4 mt-0 p-10">
                    ...
                    <div className="flex-grow">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          />
                          <Tooltip
                            formatter={(value: number, name: string, props: any) => [`R$ ${value.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, props.payload.municipio]}
                          />
                          <Bar dataKey="total" fill="#8e44ad" barSize={20} radius={[0, 10, 10, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                  ...
                  <div className="h-[600px] bg-white rounded-xl shadow p-4 flex flex-col">
                    <h2 className="text-lg font-bold text-center mb-2">
                      Agricultores por Município
                    </h2>
                    <div className="flex-grow">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          data={agricultoresPorMunicipio}
                          layout="vertical"
                          margin={{ top: 5, right: 30, left: 100, bottom: 5 }}
                        >
                          <XAxis type="number" hide />
                          ...
                          />
                          <Tooltip
                            formatter={(value: number, name: string, props: any) => [`${value} Agricultores`, props.payload.municipio]}
                          />
                          <Bar dataKey="quantidade" fill="#2ecc71" barSize={20} radius={[0, 10, 10, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
              ...
              <div className="p-10">
                ...
                <div className="flex flex-wrap items-center gap-4 mb-6">
                  ...
                  >
                    ...
                    {[...new Set(agricultoresFiltrados.map((a) => a.municipio))].map(
                      (municipio, idx) => (
                  ...
                  >
                    ...
                    {[...new Set(agricultoresFiltrados.map((a) => a.status))].map(
                      (status, idx) => (
                  ...
                  <select
                    value={campoPesquisa}
                    onChange={(e) =>
                      setCampoPesquisa(e.target.value as 'nome' | 'cpf' | 'endereco')
                    }
                    className="rounded-full px-6 py-2 border border-gray-300 shadow-sm text-sm text-gray-700"
                  >
                      ...
                      .filter(
                        (a) =>
                          ...
                          (!pesquisaTexto ||
                            a[campoPesquisa]
                              .toLowerCase()
                              .includes(pesquisaTexto.toLowerCase()))
                      )
                      .sort((a, b) =>
                        ordenacao === 'asc'
                      ...
                      agricultoresFiltrados.filter(
                        (a) =>
                          ...
                          (!pesquisaTexto ||
                            ...
                            a.endereco.toLowerCase().includes(
                              pesquisaTexto.toLowerCase()
                            ))
                      ).length / 10
                    )}
                  </span>
            ...
            {abaAtiva === 'Entidades' && (
              <div className="p-10">
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  >
                    <option value="">Todos os Municípios</option>
                    {[...new Set(entidadesFiltradas.map((a) => a.municipio))].map(
                      (municipio, idx) => (
                        <option key={idx} value={municipio as string}>
                          {municipio as string}
                        </option>
                      )
                    )}
                ...
                <table className="min-w-full bg-white rounded shadow">
                  ...
                  <tbody>
                    {[...entidadesFiltradas]
                      .filter(
                        (a) =>
                          (!filtroMunicipioEntidade ||
                            a.municipio === filtroMunicipioEntidade) &&
                          (!pesquisaEntidade ||
                            a.entidade.toLowerCase().includes(pesquisaEntidade.toLowerCase()))
                      )
                      .sort((a, b) =>
                ...
                <div className="flex justify-center mt-6 space-x-2">
                  ...
                  <span className="px-4 py-2 text-lg font-semibold">
                    Página {paginaEntidade} de{' '}
                    {Math.ceil(
                      entidadesFiltradas.filter(
                        (a) =>
                          (!filtroMunicipioEntidade ||
                            a.municipio === filtroMunicipioEntidade) &&
                          (!pesquisaEntidade ||
              ...
              <div className="p-10">
                ...
                <div className="flex flex-wrap items-center gap-4 mb-6">
                  ...
                  >
                    ...
                    {[...new Set(comprasFiltradas.map((item) => item.cooperativa))].map(
                      (coop, idx) => (
                        <option key={idx} value={coop}>
                          {coop}
                        </option>
                      )
                    )}
                  ...
                  >
                    ...
                    {[...new Set(comprasFiltradas.map((item) => item.produto))].map(
                      (prod, idx) => (
                  ...
                  >
                    <option value="">Todos os Municípios</option>
                    {[...new Set(comprasFiltradas.map((item) => item.municipio))].map(
                      (mun, idx) => (
                  ...
                  >
                    ...
                    {[...new Set(comprasFiltradas.map((item) => item.entidade))].map(
                      (ent, idx) => (
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCampoPesquisaCompras(e.target.value as 'cooperativa' | 'cnpj')
                    }
                    className="rounded-full px-6 py-2 border border-gray-300 shadow-sm text-sm text-gray-700"
                  >
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={`Pesquisar por ${
                        campoPesquisaCompras === 'cooperativa' ? 'nome' : 'CNPJ'
                      }...`}
                    ...
                    {[...comprasFiltradas]
                      .filter(
                        (item) =>
                          ...
                          (!pesquisaTextoCompras ||
                            item[campoPesquisaCompras]
                              ?.toLowerCase()
                              .includes(pesquisaTextoCompras.toLowerCase()))
                      )
                      .sort(
                        (a, b) =>
