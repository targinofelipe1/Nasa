...
import MapaParaibaRGA from "./map-rga";
import RegionalIndicators from "./indicadores";
...
export default function MapsPage() {
  ...
  useEffect(() => {
    console.log("📊 Dados da API carregados (RGA):", apiData.map(d => d.RGA));
  }, [apiData]);
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex flex-col w-full h-full p-4">
        ...
        <h1 className="text-2xl font-bold mb-4 text-center">Mapa Interativo das Regionais da Paraíba</h1>
          {apiData.length > 0 ? (
        ...
        <div className="mt-6">
          {apiData.length > 0 ? (
            <RegionalIndicators data={apiData} setIsModalOpen={setIsModalOpen} />
          ) : (
            <p className="text-center text-gray-500">Carregando indicadores regionais...</p>
          )}
        </div>
      </div>
    </div>
